...
```
###2.2 (b) The investigator would like to test simultaneously the hypothesesmean = 0 and variance = 1 aboutmean and variance. Suggest a test statistic and a test procedure, and discuss their rationale.
...
We first fit a regression to the mean to understand the conditional mean, and see that we can expect the conditional mean a daughter's height given her mother's height is 29.91744 (Beta0) + .54175 * the mother's height. 
```{r}
fit <- with(Heights,lm(dheight~mheight))
summary(fit)
ggscatter(Heights,x="dheight",y="mheight",add = "reg.line",add.params = list(color = "blue", fill = "lightgray"))
```
...
To verify the conditional variance is constant I am going to start dealing with residuals of our regression line. So, first, I create a dataset of our residuals. By squaring the residuals I can use the underlying machinery of the regression model to generate a conditional variance, and model how it changes conditioned on dheight. 

We are essentially using another regression model to the process variance of our first regression model by asking the expectation of variance given dheight. 
```{r}
data <- Heights
data$residuals <- fit$residuals
data$variance <- fit$residuals^2
ggscatter(data,x="variance",y="dheight",add = "reg.line",add.params = list(color = "blue", fill = "lightgray"))
```
